...
| 86 | Expo Go | 2026-08-26 | FAIL -> fixed, re-run pending | The pause half was right: another app taking audio focus (Brave playing an mp3) stopped ours immediately. What followed was not sane -- the bar still showed the **pause** icon (`playing: true`), the clock frozen at 0:03 / -0:01, nothing sounding, and two taps needed to recover. Cause: the driver forwarded only `error` and `didJustFinish`, so an OS-initiated pause never reached `handleStatus`. Fixed in `8c625b1`, which forwards `playing` and `isBuffering` and reads a not-playing, not-buffering tick as an interruption; four guards, each mutation-checked. **Re-run this check against the fix.** |
