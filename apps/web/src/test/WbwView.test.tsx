...
const ayahs: WbwAyah[] = [
  { ayahNumber: 1, cells: [{ surahId: 1, ayahNumber: 1, position: 1, arabic: 'بِسْمِ', translit: "bis'mi", gloss: 'In (the) name', glossLang: null, glossGroup: null, posTag: 'P', posLabel: 'Preposition', segments: [], grammarNote: 'جار ومجرور' }], textUthmani: 'x' },
];
...
const midSurahAyahs: WbwAyah[] = [
  { ayahNumber: 16, cells: [{ surahId: 2, ayahNumber: 16, position: 1, arabic: 'أُو۟لَٰٓئِكَ', translit: "ulaaika", gloss: 'Those', glossLang: null, glossGroup: null, posTag: 'PRON', posLabel: 'Pronoun', segments: [], grammarNote: null }], textUthmani: 'x' },
];

describe('WbwView', () => {
  it('renders surah name, a back-to-reader link, and ayah blocks', () => {
    render(<WbwView bookmarkedAyahs={[]} surah={surah} surahName="Al-Fatihah" ayahs={ayahs} page={1} totalPages={1} scrollAyah={null} />);
  ...
  it('renders the Bismillah banner on page 1 for a non-Fatiha/Tawba surah', () => {
    render(<WbwView bookmarkedAyahs={[]} surah={baqarah} surahName="Al-Baqarah" ayahs={midSurahAyahs} page={1} totalPages={20} scrollAyah={null} />);
  ...
  it('does NOT render the Bismillah banner on page 2+ (mid-surah pagination)', () => {
    render(<WbwView bookmarkedAyahs={[]} surah={baqarah} surahName="Al-Baqarah" ayahs={midSurahAyahs} page={2} totalPages={20} scrollAyah={null} />);
  ...
  it('renders a Go to verse VersePicker when pickerSurahs is provided', () => {
    render(
      <WbwView
        ...
        scrollAyah={null}
        bookmarkedAyahs={[]}
        pickerSurahs={[{ id: 1, name: 'Al-Fatihah', ayah_count: 7 }]}
      />,
    );
