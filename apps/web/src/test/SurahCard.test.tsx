...
const english = { name: 'Al-Fatihah', meaning: 'The Opening' };

describe('SurahCard', () => {
  it('shows the localized name, Arabic name unchanged', () => {
    render(<SurahCard surah={surah} name={{ name: 'Fotiha', meaning: null }} />);
    expect(screen.getByText('Fotiha')).toBeInTheDocument();
    expect(screen.getByText('الفاتحة')).toBeInTheDocument();
    expect(screen.queryByText('Al-Fatihah')).toBeNull();
  });

  it('renders surah number', () => {
    render(<SurahCard surah={surah} name={english} />);
